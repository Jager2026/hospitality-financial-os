...
**And there is a step before that one, which is where the rule has actually been broken: the output of a gate or a test run is never piped into `tail`, `head`, `grep`, or anything else that truncates. It goes to a file, whole.** The rule above protects against acting *after* a failure. A truncating pipe destroys the evidence *before* the failure exists — so obeying the first rule to the letter does not save you, and on 2026-09-14 it did not: nothing was re-run prematurely, the failing test was simply unreadable by the time it failed, because `| tail -12` had been typed into the command minutes earlier, when there was nothing yet to keep.

**Two unreproduced failures now, and neither has a test name.** 2026-09-02: one backend test of 344, log deleted unread, two green runs afterwards. 2026-09-14: one of 459 on a documentation-only branch, output truncated in the pipe, green on a captured re-run. Both are recorded in `IMPLEMENTATION_PLAN.md` as failures of unknown cause, because that is the whole of what can be said about them.

**A pipe that truncates is deletion, moved earlier in time.** `pnpm run gate 2>&1 | tee <file>` keeps the terminal view *and* the evidence and costs nothing; the filtering then happens against the file, as many times as needed. Filter the stream instead and there is no afterwards — the run is not repeatable, because the thing that failed was never the thing you kept.
