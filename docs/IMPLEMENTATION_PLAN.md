...
- **Environment variables that became load-bearing after being declared optional (ADR-045's class, audited on the Founder's instruction).** `ALERT_WEBHOOK_URL` is now required in production and is not the only one of its shape. Two others were found by reading every `.optional()` and `.default()` in `env.validation.ts` against what actually consumes them:
  ...
  - **`NODE_ENV`, and it is the sharpest of the three.** It carries `.default("development")`. `StripeService`'s boot-time liveness probe — ADR-038 Decision 2, the mechanism built specifically to catch the one-character secret truncation that `env.validation.ts` admits it cannot — is gated on `NODE_ENV === "production"`. **An environment that lost `NODE_ENV` would silently disable it**, along with production log levels. It also gates the new `ALERT_WEBHOOK_URL` requirement, so that guard cannot protect against its own gate being lost — stated in the code and asserted by a test rather than left implicit.
  - **`FRONTEND_URL`** defaults to `http://localhost:3000` and is read at `restaurant.service.ts:138` for Stripe Account Links `refresh_url`/`return_url`. If it were ever missing in production, a restaurant owner finishing Stripe onboarding would be redirected to localhost — no boot failure, no log, just a broken return journey at the end of the most important flow a new customer completes.
  ...
  **Not fixed on discovery, deliberately:** the Founder approved requiring `ALERT_WEBHOOK_URL` specifically, and each of these changes what a deploy does — the same reason the seed decisions above are being kept apart. The general rule they share is worth stating once: **a variable's optionality is a claim about consequences, and it stops being true the moment something starts depending on it.** Nothing re-checks that claim when the dependency is added, which is exactly how all three of these arose.
