...
**NEXT BLOCK CLOSURE: after PR #121.** Update this line when it fires.

A block closure is the five-point pass: live verification against production, permission-coverage audit, full document reconciliation, ADR contradictions, and the list of what remains unproven. The most recent one is `BLOCK_CLOSURE_110_116.md`.
...
- **Staging environment (ADR-035).** The project has exactly one Railway environment: `production`. Everything that is not a developer's laptop is live production — which is the shared root of tunnelling into the production database to verify alerting, creating throwaway test restaurants in it, and having nowhere to rehearse a restore. Its full shape is already decided (ADR-035: separate Railway environment, separate Stripe sandbox, separate JWT/webhook secrets, separate alert channel, synthetic seed with production dumps **prohibited**), so this is scheduling, not design. **Deferred with the same trigger: before the first real pilot restaurant is onboarded.** Deferred for two specific reasons, neither of which is cost — the entire production stack currently bills around $1.83/month, so a duplicated idle environment is a rounding error. It was **blocked** — that half has since expired and is kept here rather than deleted, because how it expired is the more useful record. Written on 2026-08-24, it said Stripe integration was non-functional against `invalid_v2_key` and that half of staging could not be exercised even if it existed. The cause was a `STRIPE_SECRET_KEY` truncated by one character in Railway (ADR-038), **corrected on 2026-08-30**; `POST /restaurants` was re-measured on 2026-09-07 and returns 201 with a real Stripe Connect account, and the onboarding link mints. **The sentence outlived its truth by fourteen days**, in the present tense, while ADR-038 had recorded the cause two days after it was written. What remains is **premature**: staging exists to stop us touching a database holding customer data, and there is no customer data yet. That reason expires exactly when the trigger fires.
...
- **~~Does the seed run on deploy, and may it delete?~~ ANSWERED by ADR-048 — yes, and no, respectively.** The entry below is kept because the *separation* of the two questions is the reasoning ADR-048 rests on.

  **CORRECTED 2026-09-05 (block closure #117–#156, part 1). The sentence that stood here was false, and it was false about the thing this entry exists to answer.** It read: *"`railway.backend.json` runs migrations before a deploy and nothing else, so `seed.ts` has never executed automatically."* That was true when written and stopped being true when ADR-048 merged. **`preDeployCommand` today is `prisma:migrate:deploy && prisma:seed`, and the seed runs on every deploy.**

  **What protects production is the absence of a FLAG, not the absence of a run — and those are different guarantees.** The old sentence promised that nothing could revoke because nothing ran. The real guarantee is narrower and must be stated as it is:

  - `railway.backend.json` invokes the seed **without `--allow-revocations`** — verified absent from the config, 2026-09-05.
  ...
  **So the honest form of the answer to "can a deploy silently revoke permissions in production?" is: no, and it is one line of configuration away from yes.** ADR-048 says as much — *"that word must never appear in `railway.backend.json` without its own ADR"* — and that rule has **no mechanism behind it**: nothing in CI or in the invariant suite reads `railway.backend.json` or the service's variables. It is a sentence in an ADR. **Scheduled as its own entry below — *ADR-048's flag rule has no mechanism, and only half of it can get one*** — because one half is buildable and the other is not, and folding them together would hide that.
  ...
  - **The seed runs on every deploy — DONE (ADR-048).** Makes it authoritative *on a schedule*. This was written while the seed was `upsert`-only, and that is no longer why it is safe; see the correction above.
  ...
  Each alone is defensible. **Together they would mean every deploy can silently revoke permissions in production**, which is a materially different system from either — and ADR-048's whole design is that the two are kept one line apart rather than allowed to arrive together.

  **The question the ADR was required to answer, kept because the answer is still what the guarantee rests on: if the seed is authoritative and runs on every deploy, what protects production from an incomplete `seed.ts`? Answered by ADR-048 — it fails the deploy rather than revoking.** One fact argues in favour and does not answer this — `seed.ts:135` is the **only** writer of `RolePermission` anywhere in the codebase (verified by grepping the table, not by recalling the flows), so an authoritative seed cannot be fighting some other grant path. But that establishes only that nothing else writes; it says nothing about the seed itself being wrong. **This exact failure has already happened once in this project**, in the test fixtures: `test/global-setup.ts` maintained its own copy of the matrix and had gone stale at 4 of 10 Permissions and 3 of 4 Roles. A stale matrix that can only add is a documentation bug. A stale matrix that can delete is an outage.
