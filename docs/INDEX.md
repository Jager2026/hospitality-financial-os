...
| [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) | 2.7.0 | The sprint-by-sprint build plan — tasks and Definition of Done for each sprint, Sprint 0 through the current one, plus a Deferred/Not-Yet-Scheduled backlog for flagged major-version upgrades, MFA, the off-platform backup, and the staging environment (the last two both triggered by the first real pilot restaurant). |
