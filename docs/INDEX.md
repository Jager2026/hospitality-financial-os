...
| [ARCHITECTURE_DECISIONS.md](ARCHITECTURE_DECISIONS.md) | 1.12.0 | The full ADR log — twenty-six accepted architecture decisions, each with context, decision, and consequences; the canonical record of *why*, including in-place revisions to earlier decisions (e.g. ADR-009, ADR-014, ADR-023). |
...
| [API_Contract.md](API_Contract.md) (title: API_SPECIFICATION) | 2.8.0 | Every REST endpoint — request/response shape, auth requirements, rate limits — the contract the frontend and backend both build against. |
...
| [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) | 2.2.1 | The sprint-by-sprint build plan — tasks and Definition of Done for each sprint, Sprint 0 through the current one. |
