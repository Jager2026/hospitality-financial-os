-- Ledger integrity constraints — not expressible in schema.prisma's DSL, so hand-authored.
-- Source: apps/backend/prisma/sql/ledger_integrity.sql (kept in sync — see that file for the
-- full rationale and the Sprint 0 audit references).
